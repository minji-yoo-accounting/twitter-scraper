...
df = pd.read_csv('C:/Users/minji/Desktop/Thesis/data_in/Scraped_Twitter/twitter_original_with_likes.csv')
...
if __name__ == "__main__":
    ...
    for id in id_list: 
        ...
        df = pd.DataFrame(scraper.scraped_data, columns=['likers']).drop_duplicates(keep='first')
        id_list = [id] * len(df)
        df['id'] = id_list
        df.to_sql('likers', conn, if_exists='append', index=False)
        print('{:>5,} likers added.'.format(len(df)))
