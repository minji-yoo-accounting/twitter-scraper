...
df = pd.read_csv('C:/Users/minji/Desktop/Thesis/data_in/Scraped_Twitter/influencer_replies.csv')
...
if __name__ == "__main__":
    ...
    for id in id_list: 
        ...
        id_list = [id] * len(df)
        df['original'] = id_list
        df.to_sql('influencer_replies', conn, if_exists='append', index=False) # change it
        print('{:>5,} replies added.'.format(len(df)))

    
    
    # df = pd.DataFrame(scraper.scraped_data, columns=['name']).drop_duplicates(keep='first')
    # df
